 ...
 * Summary: 对年月日改吧改吧 把日变为了周
 */
public class WheelTimeWeek {
    public static DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    public static DateFormat dateFormat2 = new SimpleDateFormat("yyyy-MM");
    ...
    public WheelTimeWeek(View view, boolean[] type, int gravity, int textSize) {
        super();
        this.view = view;
        this.type = type;
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                instance.set(year,month,31);
                //end = instance.get(Calendar.WEEK_OF_MONTH);
                end=getWeeks(year+"-"+(month+1));
                wv_day.setAdapter(new NumericWheelAdapter(startDay, end));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
                instance.set(year,month,30);
                //end = instance.get(Calendar.WEEK_OF_MONTH);
                end=getWeeks(year+"-"+(month+1));
                wv_day.setAdapter(new NumericWheelAdapter(startDay, end));
            } else {
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
               ...
               // end = instance.get(Calendar.WEEK_OF_MONTH);
                ...
                wv_day.setAdapter(new NumericWheelAdapter(1, end));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
...
//        int maxItem;
        if (list_big.contains(String.valueOf(monthNum))) {
            ...
            instance.set(year_num,monthNum-1,31);
            //endD = instance.get(Calendar.WEEK_OF_MONTH);
            endD=getWeeks(year_num+"-"+monthNum);
...
//            maxItem = endD;
            ...
            instance.set(year_num,monthNum-1,30);
            //endD = instance.get(Calendar.WEEK_OF_MONTH);
            endD=getWeeks(year_num+"-"+monthNum);
            wv_day.setAdapter(new NumericWheelAdapter(startD, endD));
//            maxItem = endD;
            ...
            if ((year_num % 4 == 0 && year_num % 100 != 0)
                ...
                instance.set(year_num,monthNum-1,29);
                //endD = instance.get(Calendar.WEEK_OF_MONTH);
                endD=getWeeks(year_num+"-"+monthNum);
                wv_day.setAdapter(new NumericWheelAdapter(startD, endD));
//                maxItem = endD;
                ...
                instance.set(year_num,monthNum-1,28);
                //endD = instance.get(Calendar.WEEK_OF_MONTH);
                endD=getWeeks(year_num+"-"+monthNum);
                wv_day.setAdapter(new NumericWheelAdapter(startD, endD));
//                maxItem = endD;
     ...
     * @param date 2013-9 对应周的日期区间： 1-1,2-8,9-15,16-22,23-29,30-30
     * @return
     * @throws Exception
     */
    public static int getWeeks(String date){
        ...
        for (int i = 1; i <= days; i++) {
            Date date2 = null;
            try {
                date2 = dateFormat3.parse(date + "-" + i);
            } catch (ParseException e) {
                LZLog.e("error","获取当前月自然周，日期格式转换错误！22");
                e.printStackTrace();
            }
            calendar.clear();
