 ...
 * Summary: 年月周  注意这里的日代表周
 */
public class WeekSelectView extends BasePickerView implements View.OnClickListener{
    private WheelTimeWeek wheelTime; //自定义控件
    private static final String TAG_SUBMIT = "确定";
    private static final String TAG_CANCEL = "取消";

    public WeekSelectView(PickerOptions pickerOptions) {
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new WheelTimeWeek(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
        if (mPickerOptions.timeSelectChangeListener != null) {
            wheelTime.setSelectChangeCallback(new ISelectTimeCallback() {
                @Override
                public void onTimeSelectChanged() {
                    try {
                        Date date = WheelTimeWeek.dateFormat.parse(wheelTime.getTime());
    ...
    public void returnData() {
        ...
        if (mSelectListener!=null){
            try {
                Date date = WheelTimeWeek.dateFormat.parse(wheelTime.getTime());
               // Date date1 = WheelTimeWeek.dateFormat.parse(wheelTime.getTime());

                mSelectListener.onTimeWeekSelect(getYearMonth(date),wheelTime.wv_day.getCurrentItem()+1, clickView);
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
    }
    public static String getYearMonth(Date  timeTemp){
    ...
    public  OnTimeWeekSelectListener mSelectListener;
    public void setOnTimeWeekSelectListener(OnTimeWeekSelectListener listener){
