 ...
 * Summary: 每日爆款--子页面(如实拍)的adapter
 */
public class DailyChildAdapter extends CommonRecyclerViewAdapter {
    private FragmentManager mFragmentManager;

    public DailyChildAdapter(Context context, FragmentManager fragmentManager) {
        super(context);
    ...
    class ViewHolder extends CommonViewHolder implements View.OnClickListener {
        ...
        public void bindData(Object itemData) {
            ...
            List<String> commodityImgList = bean.getCommodityImgList();
            List<FindCircleChildListBean.CommodityInfoListBean> commodityInfoList = bean.getCommodityInfoList();
           /* if (ArraysUtils.isListEmpty(commodityImgList)&&ArraysUtils.isListEmpty(commodityInfoList)
                    &&TextUtils.isEmpty(bean.getRingCommodityVideo())){
                //无图 无商品 无视频，则不可分享
                shareNumberTv.setVisibility(View.INVISIBLE);
            }else {
                shareNumberTv.setVisibility(View.VISIBLE);
            }*/

            shareNumberTv.setText(String.valueOf(bean.getShareNumber()));
            //设置 标签,需要转2次
            Spanned spanned = Html.fromHtml(bean.getDescContext());
            contentTv.setText(Html.fromHtml(spanned.toString()));

            //图片
            innerImageAdapter.setListData(commodityImgList);
            innerImageAdapter.notifyDataSetChanged();
            //推荐商品
            mBottomAdapter.setListData(commodityInfoList);
            ...
            if (TextUtils.isEmpty(bean.getRingCommodityVideo())) {
                ...
                if (ArraysUtils.isListEmpty(bean.getCommodityImgList())) {
                    ...
                    if (!ArraysUtils.isListEmpty(bean.getCommodityInfoList())) {
                        ...
                        //只分享文字
                        LZShare.share(LZShare.getChannelListText(
                                Html.fromHtml(Html.fromHtml(bean.getDescContext()).toString()).toString()))
                                .show();
                    }
                } else {
                    //没有视频，但有图片
