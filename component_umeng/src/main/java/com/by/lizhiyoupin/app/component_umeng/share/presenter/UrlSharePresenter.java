 ...
 * 链接分享
 */
public class UrlSharePresenter extends BaseSharePresenter implements ISharePresenter {

    public UrlSharePresenter(IShareContentManager shareContentManager) {
    ...
    void onThirdAuthComplete(ChannelListBean channelListBean, SHARE_MEDIA plat, Map<String, String> map) {
        shareToChannel(channelListBean, plat);
    }
     ...
     * @param bean     ChannelListBean
     */
    private void shareToWxCircle(final SHARE_MEDIA plat, @NonNull final UMShareListener listener, final ChannelListBean bean) {
        ...
        } else if (bean.getShareFile()!=null){
            String absolutePath = bean.getShareFile().getAbsolutePath();

            if (absolutePath.endsWith("jpg")||absolutePath.endsWith("png")){
                web.setThumb(new UMImage(context,bean.getShareFile()));
            }else{
                web.setThumb(new UMImage(context,R.mipmap.share_logo));
            }
        }else{
            web.setThumb(new UMImage(context,R.mipmap.share_logo));
        }
        web.setDescription(bean.getDesc());
        new ShareAction(context).setPlatform(plat)
                .setCallback(listener)
    ...
    private void shareToOther(final SHARE_MEDIA plat, @NonNull final UMShareListener listener, final ChannelListBean bean) {
        ...
        if (bean.getShareType() == ShareParams.TYPE_H5_URL) {
            ...
            if (bean.getShareFile() != null) {
                UMImage image = new UMImage(context, bean.getShareFile());

                String absolutePath = bean.getShareFile().getAbsolutePath();

                if (absolutePath.endsWith("jpg")||absolutePath.endsWith("png")){
                    image.setThumb(new UMImage(context,bean.getShareFile()));
                }else{
