...
import com.by.lizhiyoupin.app.component_banner.listener.OnBannerClickListener;
import com.by.lizhiyoupin.app.component_banner.listener.OnBannerListener;
import com.by.lizhiyoupin.app.loader.ImageLoaderInterface;
import com.by.lizhiyoupin.app.component_banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    //目前仅仅支持 BannerConfig.RECTANGLE_INDICATOR 类型，设置 未选中的指示器宽度
    private int indicatorUnselectedWidth=-1;
    ...
    private int mIndicatorSelectedRectangleResId = R.drawable.rectangle_radius;
    private int mIndicatorUnselectedRectangleResId = R.drawable.unselect_rectangle_radius;
    ...
    private float mPercentage = 1;//每页占比
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        indicatorUnselectedWidth = typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_unselectd_width, indicatorSize);
    ...
    /**
     * 页面宽度所占ViewPager测量宽度的权重比例，默认为1.0F
     *
     * @param percentage 每页占比 0.0~1.0
     * @return
     */
    public Banner setPageWith(float percentage) {
     ...
     * @return
     */
    public Banner setViewPageMargin(int left, int top, int right, int bottom) {
    ...
    /**
     * 目前 仅仅支持 BannerConfig.RECTANGLE_INDICATOR 类型，设置 未选中的指示器宽度
     * @param unSelectWidth
     * @return
     */
    public Banner setRectIndicationUnselectWidth(int unSelectWidth){
        this.indicatorUnselectedWidth=unSelectWidth;
        return this;
    }
    ...
    private void setBannerStyleUI() {
        ...
        switch (bannerStyle) {
            ...
            case BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE:
                ...
                setTitleStyleUI();
                break;
            case BannerConfig.RECTANGLE_INDICATOR:
                indicator.setVisibility(visibility);
                break;
        }
    }
    ...
    private void initImages() {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                ...
                bannerStyle == BannerConfig.RECTANGLE_INDICATOR) {
            createIndicator();
    ...
    private void createIndicator() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(bannerStyle == BannerConfig.RECTANGLE_INDICATOR ?
                    indicatorUnselectedWidth:mIndicatorWidth, mIndicatorHeight);
            params.leftMargin = mIndicatorMargin;
            params.rightMargin = mIndicatorMargin;
            if (i == 0) {
                imageView.setImageResource(bannerStyle == BannerConfig.RECTANGLE_INDICATOR ?
                        mIndicatorSelectedRectangleResId : mIndicatorSelectedResId);
            } else {
                imageView.setImageResource(bannerStyle == BannerConfig.RECTANGLE_INDICATOR ?
                        mIndicatorUnselectedRectangleResId : mIndicatorUnselectedResId);
            }
...
//        Log.i(tag,"currentItem: "+currentItem);
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
                ...
                bannerStyle == BannerConfig.RECTANGLE_INDICATOR ||
                bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
            ImageView unImageView = indicatorImages.get((lastPosition - 1 + count) % count);
            ImageView imageView = indicatorImages.get((position - 1 + count) % count);
            if (bannerStyle == BannerConfig.RECTANGLE_INDICATOR) {
                ViewGroup.LayoutParams unLayoutParams = unImageView.getLayoutParams();
                unLayoutParams.width = indicatorUnselectedWidth;
                unImageView.setLayoutParams(unLayoutParams);
                unImageView.setImageResource(mIndicatorUnselectedRectangleResId);
                //顺序不能变，需要 setLayoutParams
                ViewGroup.LayoutParams layoutParams = imageView.getLayoutParams();
                layoutParams.width = mIndicatorWidth;
                imageView.setLayoutParams(layoutParams);
                imageView.setImageResource(mIndicatorSelectedRectangleResId);

            } else {
                unImageView.setImageResource(mIndicatorSelectedResId);
